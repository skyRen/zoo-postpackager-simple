...
//added by rsk
/*******  用于获取转换相对路径为绝对路径的参照路径，记录当前file的path  ******/
    ...
    var replaced = content.replace(reg, function (m, $1, $2, $3, $4) {
        ...
        if ($1) {
            ...
            if (!result || !(result[1] || result[2])) {
                ...
                /** aad by rsk **/
                isRelative && (jsUrl = fis.util.getAbsolutePath(jsUrl, currentFilePath));
                /** end */
            ...
            if (result && (result[1] || result[2])) {
                ...
                /** aad by rsk **/
                isRelative && (cssUrl = fis.util.getAbsolutePath(cssUrl, currentFilePath));
                /** end */
...
module.exports = function (ret, conf, settings, opt) { //打包后处理
    ...
    fis.util.map(ret.src, function (subpath, file) {
        if (file.isHtmlLike && file.noMapJs !== false) { //类html文件
            ...
            if(isRelative){
                //<script>
                content = content.replace(/(<script(?:(?=\s)[\s\S]*?["'\s\w\/\-]>))([\s\S]*?)(?=<\/script\s*>|$)/ig, function(m, $1){
                    if($1){
                        $1 = $1.replace(/(\s(?:data-)?src\s*=\s*)('[^']+'|"[^"]+"|[^\s\/>]+)/ig, function(m, prefix, value){
                            return prefix + '"' + fis.util.getRelativePath(value.substring(1, value.length-1), currentFilePath) + '"';
                        });
                        m = $1;
                    }
                    return m;
                ...
                //<link>
                content = content.replace(/(<link\s+[\s\S]*?["'\s\w\/\-](?:>|$))/ig, function(m, $1){
                    if($1){
                        $1 = $1.replace(/(\s(?:data-)?href\s*=\s*)('[^']+'|"[^"]+"|[^\s\/>]+)/ig, function(m, prefix, value){
                            return prefix + '"' + fis.util.getRelativePath(value.substring(1, value.length-1), currentFilePath) + '"';
                        });
                        m = $1;
                    }
